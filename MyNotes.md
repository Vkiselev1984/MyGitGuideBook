...
В предлагаемой Винцентом Дриззенем конфигурации проекта содержится один центральный репозиторий origin, куда свои изменения публикуют Алиса, Боб, Девид и Клер.

Однако они также взаимодействуют друг с другом. 
Технически Алиса создаёт удалённую ветку Git под названием bob, которая указывает на репозиторий Боба, а Боб делает то же самое с её репозиторием.

В отличие от главных ветвей (master и develop), вспомогательные ветви (features) всегда имеют ограниченный срок жизни. 
